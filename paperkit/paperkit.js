...
Paperkit.prototype.addMDMethods = function(element) {
  ...
  if (tag.indexOf("md-") >= 0) {
    if (element.alreadyInitialized) { return; }
...
function transitionEndEventName() {
  ...
  for (i in transitions) {
    if (transitions.hasOwnProperty(i) && el.style[i] !== undefined) { return transitions[i]; }
  }
}
...
var initMDInputList = function(MDInputList) {
  ...
  MDInputList.unsetSelected = function(tile) {
    if (this.hasAttribute("md-required") && this.querySelectorAll("md-tile[selected]").length <= 1) {
      return;
    };
